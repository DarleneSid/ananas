...
/*   Updated: 2023/11/09 17:35:45 by dsydelny         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_texture(t_data *d, t_tex *tex)
{
	...
	if (!tex[N].img || ! tex[S].img || !tex[E].img || !tex[W].img)
		return (printf("Oops wrong image !\n"), 1);
	...
	if (!tex[N].add || !tex[S].add || !tex[E].add || !tex[W].add)
		return (printf("Oops wrong data image !\n"), 1);
	return (0);
}
...
void	init_game(t_data *d)
{
	...
	if (!d->mlx)
	{
		printf("Oops MLX won't init...\n");
		close_window(d);
	}
	d->win = mlx_new_window(d->mlx, WIDTH, HEIGH, "cub3D");
	if (!d->win)
	{
		printf("Oops MLX won't open window...\n");
		close_window(d);
	}
